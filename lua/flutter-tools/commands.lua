...
local function get_project_root(target)
  ...
  if target then
    ...
    if project_root then return project_root.filename end
  end
  return cwd
end
...
local function run(opts, project_conf)
  ...
  executable.get(function(paths)
    ...
    local project_conf_copy = shallow_copy(project_conf)
    if cwd ~= lsp_root then
      project_conf_copy.target = Path:new(lsp_root, target):make_relative(cwd)
    end
    local args = opts.cli_args or get_run_args(opts, project_conf_copy)
